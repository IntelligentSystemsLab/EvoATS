...
public class Move extends ComplexChange {
	
	public Move() {
		super();
	}
	
	public Move(String id, String name, List<String[]> changeValues) {
		super(id, name, changeValues);
	}
	
	//This is the implementation from ChgAttValue !!! (need to adapted)
	public String getSimpleHTMLRepresenation() {
		...
		if (oldValues.length==1) {
			result += "<FONT COLOR=\"#FF0000\"><i " + this.getNameAsToolTip(oldValues[0]) + ">"+oldValues[0]+"</i></FONT>, ";
		} else {
			result += "[";
		...
		if (newValues.length==1) {
			result += "<FONT COLOR=\"#008000\"><i " + this.getNameAsToolTip(newValues[0]) + ">"+newValues[0]+"</i></FONT>)";
		} else {
			result += "[";
			for (String newValue : newValues) {
				result += "<FONT COLOR=\"#008000\"><i " + this.getNameAsToolTip(newValue) + ">"+newValue+"</i></FONT>, ";
			}
			result = result.substring(0, result.length()-2);
			result += "])";
		}
