...
import com.ats.evo.parser.ActionParser;
import com.ats.evo.parser.RuleParser;
...
import com.ats.evo.utils.DiffUtil;
...
public class DiffExecutor {
	...
	public void loadRules(String fileLocation) {
		...
		p.importRules();
		this.rules = p.rulesToImport;//from rule.xml
		for (int i=0;i<this.rules.size();i++) {
			this.rules.get(i).buildSQLStatements();
			//getChangeAction:找到constraint中对应的input
		}
	}
...
//		markingSameActions();
	...
	private void applyRule(Rule r) {
		try {
			...
			if (r.fullActionSQLStatement!=null) {
				rs = DataBaseHandler.getInstance().executeSelect(r.fullActionSQLStatement); //所有此规则的变换
				List<InputAction> compositeActions = r.getCompositeInputActions();// 由哪些低级差异组成
	...
	private void markingSameActions() {
		...
		try { //TODO:mapObj?
			ResultSet rs = DataBaseHandler.getInstance().executeSelect("SELECT value1,value2 FROM "+Globals.WORKING_TABLE+" WHERE change_action='mapObj' AND value1 != value2"); //第1个版本中一个实体映射为第2个版本另一个实体，则为重要实体
			while (rs.next()) {
